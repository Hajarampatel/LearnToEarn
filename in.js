...
app.get("/about", function (req, res) {
    res.render("about.ejs", { successl: succl, successr: succr, successc: succc });
});
...
app.get("/adminpage_student", (req, res) => {
    if (req.cookies.admin) {
        pool.query(
            ...
            function (err, res1) {
                pool.query(
                    "select *,(select count(*) from enrolls where enrolls.stu_id=student.stu_id) as count from student",
                    function (err, result) {
                        if (err) {
                            throw err;
                        } else {
        ...
        if (err) {
            ...
            pool.query(sql, function (err, result) {
                if (err) {
                    ...
                    res.render("student_dashboard", { result: result , mass : req.cookies.student});
                }
            });
        }
    });
...
app.get("/student_profile",(req,res)=>{
    ...
    {
        var uname=req.cookies.student;
        var sql='select *,(select count(*) from enrolls where enrolls.stu_id=student.stu_id) as a1,(select count(*) from givequiz where givequiz.stu_id=student.stu_id) as a2 from student where user_name="'+uname+'"';
        pool.query(sql,function(err,res1){
            console.log(res1);
            res.render("student_profile",{result:res1});
        });
    }
    else{
        res.render("login",{successl: "Login Is Required",
            successr: " ",
            successc: " "});
    }
});
...
app.get("/student_dashboard",(req,res)=>{
    if(req.cookies.student)
    {
            pool.query("SELECT * FROM e_learning.course",
            function (err, result) {
                if (err) {
                    throw err;
                } else {
                    pool.query('select stu_name from student where user_name="'+req.cookies.student+'"',function(err,r){
                        if(err) throw err;
                        else
                        res.render("student_dashboard", { result: result,mass: r[0].stu_name});
                    });
...
app.get("/student_courses",(req,res)=>{
    if(req.cookies.student)
    {
        pool.query('select stu_id,stu_name from student where user_name="'+req.cookies.student+'"',function(err,res1){
        pool.query('select * from course where course_id in (select course_id from enrolls where stu_id="'+res1[0].stu_id+'")',function(err,res2){
            if(err) throw err;
            else{
            console.log(res2[0]);
            res.render('student_courses',{result:res2,res1:res1});
            }
        });
        });
    }
...
app.get("/student_editprofile",(req,res)=>{
    if(req.cookies.student)
    {
        pool.query('select stu_id,stu_name,contact_no,student.user_name,user_password from student join account where student.user_name=account.user_name and student.user_name="'+req.cookies.student+'"',
        function(err,res1){
            res.render('student_editprofile',{result:res1});
        });
    }
    else{
        res.render("login.ejs",{successl: " ",
    ...
    upload(req, res, (err) => {
        if (err) {
            ...
            var sql1 =
                ...
                '" where stu_id="' +
                req.body.studentid +
                '";Update account set user_password="' +
                req.body.password +
                '" where user_name="' +
                req.body.username +
                '"';
